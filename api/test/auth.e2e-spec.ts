...
describe('AuthController (e2e)', () => {
  ...
  describe('/auth/register (POST)', () => {
    it('it should register a user and return a new user object', () => {
    ...
    it('it should not register a new user if an email already exists', () => {
      return request(authUrl)
        .post('/register')
        .set('Accept', 'application/json')
        .send(mockUser)
    ...
    it('it should log in and return a JWT for an registered user', () => {
      return request(authUrl)
        .post('/login')
        .set('Accept', 'application/json')
        .send(mockUser)
        .expect((response: request.Response) => {
          ...
          expect(jwt.verify(token, 'oluwajoba')).toBeTruthy();
        })
